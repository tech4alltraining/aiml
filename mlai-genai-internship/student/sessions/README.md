# Sessions

The course runs as **twelve sessions**, with Session 5 split into three parts. Each one is self-contained: read it, run the code, do the exercises, check yourself with the MCQs.

| # | Session | Notebook |
|---|---|---|
| 1 | [Python Refresher](session-01-python-refresher.md) | [open](../notebooks/session-01-python-refresher.ipynb) |
| 2 | [Python Libraries: NumPy, Pandas & Visualisation](session-02-numpy-pandas.md) | [open](../notebooks/session-02-numpy-pandas.ipynb) |
| 3 | [EDA & Data Preprocessing](session-03-eda-preprocessing.md) | [open](../notebooks/session-03-eda-preprocessing.ipynb) |
| 4 | [Introduction to AI & Machine Learning](session-04-intro-ml-ai.md) | [open](../notebooks/session-04-intro-ml-ai.ipynb) |
| 5 | [Regression](session-05-regression.md) | [open](../notebooks/session-05-regression.ipynb) |
| 5B | [Classification](session-05b-classification.md) | [open](../notebooks/session-05b-classification.ipynb) |
| 5C | [Model Deployment & Streamlit](session-05c-deployment.md) | [open](../notebooks/session-05c-deployment.ipynb) |
| 6 | [Augmentation, Feature Engineering & Reduction](session-06-augmentation-feature-engg-red.md) | [open](../notebooks/session-06-augmentation-feature-engg-red.ipynb) |
| 7 | [Unsupervised Learning: Clustering](session-07-unsupervised.md) | [open](../notebooks/session-07-unsupervised.ipynb) |
| 8 | [Model Evaluation & Improvement](session-08-evaluation-tuning.md) | [open](../notebooks/session-08-evaluation-tuning.ipynb) |
| 9 | [Deep Learning](session-09-deep-learning.md) | [open](../notebooks/session-09-deep-learning.ipynb) |
| 10 | [Generative AI & LLMs](session-10-genai-llms.md) | [open](../notebooks/session-10-genai-llms.ipynb) |
| 11 | [AI-Powered Applications](session-11-ai-apps.md) | [open](../notebooks/session-11-ai-apps.ipynb) |
| 12 | [Open Source, Hugging Face & Responsible AI](session-12-opensource-ethics.md) | [open](../notebooks/session-12-opensource-ethics.ipynb) |

*14 of 14 sessions published.*

---

## What every session contains

```text
🎯 Learning outcomes     what you will be able to do afterwards
🧠 Analogy               the everyday comparison for each idea
📘 Examples              2-3 worked examples per topic, all runnable
✏️ Practice              5 short exercises per topic, with solutions
❓ MCQs                  multiple-choice checks, with answers and why
🎯 Tasks                 2-3 longer pieces for after the session
```

## How to work through a session

1. Read the session page for the idea and the analogy.
2. Open the notebook and run the examples.
3. Do the five practice exercises **before** looking at the solutions.
4. Answer the MCQs from memory. Anything wrong, go back to that topic.
5. Do the tasks in your own time.

> **If you get stuck for more than ten minutes**, look at the solution for *that step only*, understand it, close it, and continue.

## Before Session 1

Set up your environment: [Setup Guide](../setup-guide.md). Something broken: [Troubleshooting](../troubleshooting.md).
